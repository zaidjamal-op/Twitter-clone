...
function Input() {
    ...
    return (
        <div className={`border-b border-gray-700 p-3 flex space-x-3 overflow-y-scroll no-scrollbar ${loading && 'opacity-60'}`}>
            <img className="h-16 w-16 rounded-full cursor-pointer" src={session?.user?.image} alt=""/>
            <div className="w-full divide-y divide-gray-700">
                <div className={`${selectedFile && "pb-7"} ${input && 'space-y-2.5'}`}>
                    <textarea value={input} onChange={(e) => setInput(e.target.value)} rows="2" className="bg-transparent outline-none text-[#d9d9d9] placeholder-gray-500 w-full min-h-[50px]" placeholder="What's happening?"/>
                    {selectedFile && (
