...
function Post({ id, post, postPage }) {
  ...
  const [loading, setLoading] = useState(false);
    ...
    useEffect(
        () => setLiked(likes.findIndex((like) => like.id === session?.user?.uid) !== -1), [likes]
    )
  ...
  return (
    <div className="p-3 flex cursor-pointer border-b border-gray-700" onClick={() => router.push(`/${id}`)}>
      ...
      <div className="flex flex-col space-y-2 w-full">
        ...
        {likes.length > 0 && (
              <span onClick={(e) => {e.stopPropagation(); router.push('/likes'); localStorage.setItem('postid', id)}}
                className={`hover:text-pink-600 text-sm ml-2 hover:underline ${
                  liked && "text-pink-600"
                }`}
              >
                {likes.length} Likes
